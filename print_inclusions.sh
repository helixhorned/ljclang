...
if [[ -z "$compiler_arg" || -z "$compile_commands_file" ]]; then
	exec >&2
	echo "Usage: $0 [-j<concurrency>] <compiler> <compile_commands.json> [--exclude=<regex>]"
	echo
	...
	echo "   (currently: as generated by CMake). From this argument, the"
	echo "   project directory is extracted as described below."
	echo "   In the referenced file, the final element of each 'command'"
	echo "   string must start with the canonicalized project directory."
	echo "- <regex> is matched against the full command after tweaking"
	echo "   (see Note below)."
	echo
	echo "The project directory is extracted from the value <compile_commands.json>"
	echo "as follows:"
	echo " - if it contains '//' (which may be at most once), the part before;"
	echo " - otherwise, the path obtained by dirname(1)."
	echo
...
if [[ "$compile_commands_file" =~ ^(.*/)(/.*)$ ]]; then
	...
	if [[ "$first" =~ // || "$second" =~ // ]]; then
		echo "ERROR: <compile_commands.json> may contain at most one '//'." >&2
		exit 1
	fi
	project_dir="${first%/}"
elif ! project_dir=$(dirname "$compile_commands_file"); then
...
if [ -n "$exclude_regex_arg" ]; then
	if [[ "${exclude_regex_arg:0:10}" != '--exclude=' ]]; then
		echo "ERROR: malformed first optional argument, expecting it to start with '--exclude='." >&2
		exit 1
	fi

	exclude_regex=${exclude_regex_arg:10}
fi
...
dropped_warn_count=0

for ci in "${!commands[@]}"; do
	command="${commands[$ci]}"

	if [[ -n "$exclude_regex_arg" && "$command" =~ $exclude_regex ]]; then
		dropped_note_count=$((dropped_note_count + 1))
		continue
	fi

	if [ "${command:0:$compiler_len+1}" != "$compiler " ]; then
		dropped_warn_count=$((dropped_warn_count + 1))
		continue
	fi
...
if [[ "$dropped_note_count" -gt 0 ]]; then
	echo "NOTE: dropped $dropped_note_count commands due to '--exclude=<regex>' filter." >&2
fi

if [[ "$dropped_warn_count" -gt 0 ]]; then
	echo "WARNING: dropped $dropped_warn_count commands not starting with '$compiler '." >&2
fi

if [[ "$new_command_count" -eq 0 ]]; then
	echo "ERROR: no commands remain to process." >&2
	exit 1
fi
